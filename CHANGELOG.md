...
The dying-worker successor handoff now serves only CLI-initiated `llm-mem restart`, where the running install *is* the resolved install.
...
### Worker & providers
- `LLM_MEM_MAX_CONCURRENT_AGENTS` actually enforced via atomic slot reservations (#3294)
...
- `LLM_MEM_TIER_SUMMARY_MODEL` honored on OpenAI-compatible providers (#3257)
...
- `LLM_MEM_EXCLUDED_PROJECTS` honored on session-start injection (#3358); subagents without MCP tools skip file-context injection (#3341)
- `~` expanded in `LLM_MEM_DATA_DIR` (#3350) and `CLAUDE_CODE_PATH` (#3275); Homebrew `uvx` path shared with the worker preflight (#3276)
...
### Docs
- New [`docs/merge-rubric.md`](https://github.com/ChenhyW/llm-mem/blob/main/docs/merge-rubric.md) — the acceptance bar for bug-fix PRs, distilled from this sweep.
...
This release ships the complete two-lane sync architecture between your local llm-mem database and the cmem.ai sync hub (PR #3333):
...
**Sync is OFF by default.** `LLM_MEM_CLOUD_SYNC_HUB_URL` defaults to empty — nothing leaves your machine unless you configure a hub URL (see the `cloud-sync` skill or https://docs.llm-mem.ai/cloud-sync).
...
## Fixes
- **Worker host**: clients now honor `LLM_MEM_WORKER_HOST` (the address the server actually binds), with IPv6 literals bracketed correctly in health checks and display URLs.
...
Google deprecated Gemini CLI's free/individual tier (cutoff June 18, 2026) in favor of **Antigravity CLI**, the official successor announced May 19, 2026. This release migrates llm-mem accordingly.

### Removed
- Gemini CLI host integration (adapter, installer, IDE-detection entry, hooks, dedicated docs/tests). The separate, still-supported Gemini LLM/observation provider (`LLM_MEM_GEMINI_API_KEY`, `GeminiProvider`) is unaffected.
...
- `npx llm-mem antigravity-cli install|status|uninstall` subcommand support.
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v13.9.2...v13.9.3
...
- `LLM_MEM_{GEMINI,OPENROUTER}_MAX_CONTEXT_MESSAGES` / `_MAX_TOKENS` settings, defaults, and validation
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v13.9.0...v13.9.1

## [13.9.0] - 2026-06-29

## Highlights

### 🚀 New: \`llm-mem/sdk\` (cmem-sdk)
A fully in-process capture → compress → semantic-search pipeline with **no HTTP worker and no Redis**. Import \`createCmemClient\` from \`llm-mem/sdk\`, point it at Postgres + a running \`uvx chroma-mcp\` + an LLM provider, and call \`capture\`/\`generate\`/\`search\`/\`context\`/session methods directly.
...
Full changes: https://github.com/ChenhyW/llm-mem/pull/3017
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v13.7.0...v13.7.1

## [13.7.0] - 2026-06-20

## PostHog telemetry overhaul

A ground-up rebuild of llm-mem's telemetry — per-session rollups, unified instrumentation, and real (redacted) error tracking. Grounded in live PostHog data: the raw-event volume was confirmed to be **legacy-fleet decay** (raw `session_compressed` fell ~75% in two days as installs updated), so this is the proper rebuild, not a hotfix.
...
- **New kill-switch** — `LLM_MEM_TELEMETRY_ERRORS=0` disables error capture independently of analytics.
...
### Privacy
This release begins collecting redacted error messages/stacks (a deliberate, consent-gated shift from whitelist-only telemetry). Raw paths, prompts, project names, source code, and model output are still never collected. Opt out of all telemetry with `LLM_MEM_TELEMETRY=0` / `DO_NOT_TRACK=1`, or errors-only with `LLM_MEM_TELEMETRY_ERRORS=0`.
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v13.6.1...v13.6.2
...
### Privacy & safety
- Honors the exact same consent gates as live telemetry: `DO_NOT_TRACK`, `LLM_MEM_TELEMETRY=0`, and `telemetry.json` opt-out. Opting out before your first post-upgrade worker start prevents the backfill entirely; a later opt-in still backfills.
...
- `LLM_MEM_TELEMETRY_DEBUG=1` dry-runs the full payload to stderr without sending anything.
...
Full disclosure documented at [docs.llm-mem.ai/telemetry](https://docs.llm-mem.ai/telemetry).
...
- **Probes every candidate for capability**, not just existence: each CLI is tested with `--permission-mode dontAsk --version`, the exact flags llm-mem passes on every agent spawn. Binaries that reject them (older than the 2.1.x line) are skipped up front with a clear warning.
...
- **Test hygiene** — the test suite can no longer touch the real `~/.llm-mem` (a preload tripwire isolates every run), ending sentinel-PID and corrupt-JSON pollution of production state.
...
llm-mem instrumented success well — failure was invisible. This release adds the five highest-value missing reliability signals (#2874). Everything is closed-enum/count-only, whitelisted in the scrubber, and disclosed in both the [public docs](https://docs.llm-mem.ai/telemetry) and `llm-mem telemetry`.
...
- The telemetry docs (https://docs.llm-mem.ai/telemetry) and the `npx llm-mem telemetry enable` consent screen now disclose the ingest-derived coarse location.
...
Every analytics number llm-mem reports about itself is now real, provider-reported data — plus a new daily install-state snapshot so we can see the actual state of the installed base.
...
All new properties are whitelisted in the scrubber, documented at https://docs.llm-mem.ai/telemetry, and shown in the `npx llm-mem telemetry` consent screen. Telemetry remains anonymous and opt-out (`npx llm-mem telemetry disable`).

## [13.5.2] - 2026-06-10

## What's New in 13.5.2

Platform and toolchain telemetry to diagnose the install → live-worker activation dropoff (anonymous, opt-out — see `npx llm-mem telemetry`):
...
Deep telemetry instrumentation (anonymous, opt-out — see `npx llm-mem telemetry`):
...
llm-mem now ships anonymous, privacy-hardened usage analytics. This is the first release with any telemetry, and it follows the standard dev-tool model (Homebrew, Next.js, Astro): **on by default, one command to opt out, and incapable of carrying your content by construction.**
...
- `npx llm-mem telemetry disable`
- `DO_NOT_TRACK=1` (the universal standard — overrides everything)
- `LLM_MEM_TELEMETRY=0`

`npx llm-mem telemetry status` shows the current state and which setting decided it. The installer asks once at the end of `npx llm-mem install`, and your answer is never re-asked.

Full documentation of every field and event: https://docs.llm-mem.ai/telemetry

### Also in this release

- Install flow: live progress for dependency steps and a consent prompt at the end of install
- `npx llm-mem telemetry [status|enable|disable]` CLI command
...
**Installer: \"work email\" opt-in** — the CMEM Online signup prompt in `npx llm-mem install` now asks for your *work* email (placeholder `you@company.com`). This surfaces which orgs are adopting llm-mem.

## [13.4.1] - 2026-06-08

## What's new

### 🟣 CMEM Online email opt-in during `npx llm-mem install`
...
- POSTs to the live `https://cmem.ai/api/waitlist` endpoint (handles persistence, dedup, and the confirmation email server-side). Overridable via `LLM_MEM_SIGNUP_URL`; tagged `source: npx-installer`.
- Skipped automatically when non-interactive, under CI, or with `LLM_MEM_ONLINE_OPTIN=false`.
...
### Features
- **Configurable OpenAI-compatible base URL** for the OpenRouter provider (`LLM_MEM_OPENROUTER_BASE_URL`) — point llm-mem at DeepSeek, LM Studio, or any custom OpenAI-compatible endpoint.
...
Full diff: https://github.com/ChenhyW/llm-mem/pull/2701
...
- **weekly-digests** (#2399) — produces a chapter-per-ISO-week serial digest of a project's full llm-mem timeline. Sequential subagent pipeline keeps the narrative coherent across 30+ chapters.
...
- Add `LLM_MEM_ENV_FILE` lazy resolver so tests (and multi-profile users) can redirect the env-file path without module-load-order constraints
...
### Tests / review
- `tests/env-isolation.test.ts` no longer mutates the real `~/.llm-mem/.env`; OAuth spy wrapped in try/finally to avoid leaks across runs
- 3 new chroma-mcp regression tests for #2313 (singleton enforcement)

### Misc
- Daily dependency bump per CLAUDE.md maintenance policy

Full diff: https://github.com/ChenhyW/llm-mem/pull/2394

## [13.0.0] - 2026-05-08

## Highlights

This is the **llm-mem 13** major release, landing the Server Beta runtime and the project's relicense.

### Server Beta runtime (opt-in)
- Independent server-beta service with its own lifecycle (`llm-mem server start/status/stop`)
- Postgres-backed observation storage
- BullMQ + Redis observation queue engine (gated behind `LLM_MEM_QUEUE_ENGINE=bullmq`, fail-fast)
...
Full diff: https://github.com/ChenhyW/llm-mem/compare/v12.7.5...v13.0.0
...
Fixes:
- If Codex already has llm-mem-local registered from a different source, the installer now removes that stale registration and re-adds the local npx marketplace instead of failing.
...
- npx --yes llm-mem@12.7.5 --version
...
Highlights:
- Restores Codex access to the llm-mem MCP/search plugin by pointing the Codex marketplace at the bundled plugin root.
...
## Verification
- `npm run build`
- `npm publish` completed for `llm-mem@12.7.1`
...
- Add Codex installer support for `npx llm-mem@latest install` with Codex CLI version guidance.
...
## Fix: `npx llm-mem@latest install` no longer hangs on tree-sitter-swift

### What broke in 12.6.1

PR #2300 moved 21 tree-sitter grammar packages from root `devDependencies` → root `dependencies`. As a result, `npx llm-mem@12.6.1 install` started fetching all 21 grammars at npx time. `tree-sitter-swift`'s postinstall pulled a nested `tree-sitter-cli` that downloads a Rust binary from GitHub and SIGINT'd the install:

```
npm error path .../node_modules/llm-mem/node_modules/tree-sitter-swift/node_modules/tree-sitter-cli
...
- **F2 `paths`** (`src/shared/paths.ts`) — 24 hardcoded `homedir() + '.llm-mem'` sites collapsed into 18 named accessors. `LLM_MEM_DATA_DIR` flows through 100% of runtime. Self-extending invariant test.
...
- #2242 — `check-pending-queue.ts` points at `/api/processing-status` + `/api/processing`; honors `LLM_MEM_WORKER_PORT`
...
### Highlights
- Multi-account isolation via `LLM_MEM_DATA_DIR` + per-UID worker port (`37700 + uid % 100`), with `LLM_MEM_WORKER_PORT` override (#2101)
- New `LLM_MEM_INTERNAL=1` trust boundary replaces cwd-based observer-session detection
...
- `opencode-plugin` validates `LLM_MEM_WORKER_PORT` before use
...
**One-shot DB cleanup migration** (`CleanupV12_4_3.ts`) — runs once per data directory at worker startup, marker-file gated, opt-out via `LLM_MEM_SKIP_CLEANUP_V12_4_3=1`. Cleans:
...
- `~/.llm-mem/chroma/` and `chroma-sync-state.json` so `backfillAllProjects` rebuilds vectors from the cleaned SQLite.
...
### Notes
- Cleanup of polluted rows is included in the worker — fresh installs are clean. To clean an existing DB: `sqlite3 ~/.llm-mem/llm-mem.db "DELETE FROM user_prompts WHERE prompt_text LIKE '<task-notification>%';"` (the AFTER-DELETE trigger handles FTS).
...
### What changed
- New `~/.llm-mem/chroma-sync-state.json` watermark cache — per-project highest synced sqlite_id for observations, summaries, and prompts.
...
- Five env vars control behavior. `LLM_MEM_TELEGRAM_ENABLED` master toggle defaults on (no-op without bot token + chat ID).
...
- Hooks now resolve endpoint with the same precedence as the worker: env (`LLM_MEM_WORKER_PORT`, `LLM_MEM_WORKER_HOST`) > settings.json > defaults.
...
### 📝 Docs
- `version-bump` skill now covers `npm publish` + all 6 manifest paths so `npx llm-mem@<version>` always resolves. Adds `git grep` pre-flight for new manifests.
...
- Stopped tracking `.docker-blowout-data/llm-mem.db` and added the directory to `.gitignore`.

## Full Changelog
https://github.com/ChenhyW/llm-mem/compare/v12.3.6...v12.3.7
...
- Nounset-safe `TTY_ARGS` in `docker/llm-mem/run.sh`
...
### Upgrade notes
- `~/.llm-mem/worker-auth-token` from a previous 12.3.3 install is harmless and can be deleted.
...
v12.3.3 (Issue Blowout 2026, PR #2080) broke SessionStart context injection — new sessions received no memory context from llm-mem. This release reverts to the v12.3.2 tree state while the regression is investigated.
...
- **McpIntegrations**: Goose YAML updater now throws when llm-mem markers exist but regex replacement fails
...
- Added README for Docker llm-mem harness

## [12.3.0] - 2026-04-20

## New features

### Basic llm-mem Docker container (`docker/llm-mem/`)
A ready-to-run container for ad-hoc llm-mem testing with zero local setup beyond Docker.
...
- Persistent `.llm-mem/` mount so the observations DB survives container exit
...
Two-container split (our agent image + the upstream SWE-bench harness) for measuring llm-mem's effect on resolve rate.

- `Dockerfile.agent` → `llm-mem/swebench-agent:latest` (same non-root, version-pinned approach)
...
**PR:** https://github.com/ChenhyW/llm-mem/pull/2076
...
**Full diff:** https://github.com/ChenhyW/llm-mem/compare/v12.2.2...v12.2.3
...
- **`npx llm-mem adopt`** — new CLI command with `--dry-run` and `--branch X` flags for manual adoption
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v12.1.6...v12.2.0
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v12.1.2...v12.1.3
...
Build queryable AI "brains" from your llm-mem observation history. Compile a filtered slice of your past work into a corpus, prime it into a Claude session, and ask questions conversationally — getting synthesized, grounded answers instead of raw search results.
...
- **CorpusBuilder** — searches observations, hydrates full records, calculates stats, persists to `~/.llm-mem/corpora/`
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v12.0.1...v12.1.0
...
- **CLAUDE.local.md support** — `LLM_MEM_FOLDER_USE_LOCAL_MD` setting for writing to local-only config
...
### Windows
- Fix `isMainModule` CJS branch failure on Bun — add `LLM_MEM_MANAGED` fallback
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v11.0.1...v12.0.0

## [11.0.1] - 2026-04-06

**Patch release** — Changes `LLM_MEM_SEMANTIC_INJECT` default from `true` to `false`.
...
- Users can re-enable via `LLM_MEM_SEMANTIC_INJECT=true` in `~/.llm-mem/settings.json`
...
This release consolidates v10.7.0 through v11.0.0, all shipped on April 4, 2026. For the full v10.x era (267 commits, 39 releases), see [v10.7.0](https://github.com/ChenhyW/llm-mem/releases/tag/v10.7.0) and earlier.
...
- Install command now delegates to native Claude Code plugin system: `claude plugin marketplace add ChenhyW/llm-mem && claude plugin install llm-mem`
...
### New Features
- **Timeline Report Skill** — New `/timeline-report` skill generates narrative "Journey Into [Project]" reports from llm-mem's development history with token-aware economics
...
### Tests
- Rewrote supervisor index tests to use temp directories instead of relying on real `~/.llm-mem/worker.pid`
...
- Removed dead `LLM_MEM_CONTEXT_OBSERVATION_TYPES` and `LLM_MEM_CONTEXT_OBSERVATION_CONCEPTS` settings constants
...
Adds `law-study` — a purpose-built llm-mem mode for law students.
...
- **Chroma toggle** — `LLM_MEM_CHROMA_ENABLED` setting allows SQLite-only fallback mode (#707)
...
- Fixed session context footer to reference the llm-mem skill instead of MCP search tools for accessing memories
...
2. **Periodic reaping:** Added `reapStaleSessions()` to `SessionManager` — a background interval that scans `~/.llm-mem/observer-sessions/` for stale PID files, verifies the process is still running, and kills any orphans with SIGKILL escalation
...
- **Fixed critical collection routing bug** — Backfill now uses the shared `cm__llm-mem` collection (matching how DatabaseManager and SearchManager operate) instead of creating per-project orphan collections that no search path reads from.
...
- **Moved HuggingFace model cache to `~/.llm-mem/models/`** outside `node_modules`. The ~23MB ONNX model was stored inside `node_modules/@huggingface/transformers/.cache/`, so any reinstall or cache clear corrupted it.
...
- **Cleaner out-of-box experience** — New installs now default to a streamlined context display:
  - Read tokens column: hidden (`LLM_MEM_CONTEXT_SHOW_READ_TOKENS: false`)
  - Work tokens column: hidden (`LLM_MEM_CONTEXT_SHOW_WORK_TOKENS: false`)
  - Savings amount: hidden (`LLM_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT: false`)
  - Full observation expansion: disabled (`LLM_MEM_CONTEXT_FULL_COUNT: 0`)
  - Savings percentage remains visible by default

  Existing users are unaffected — your `~/.llm-mem/settings.json` overrides these defaults.
...
- Added multi-tenancy support for llm-mem Pro
...
Claude-mem now has an official [OpenClaw](https://openclaw.ai) plugin, bringing persistent memory to agents running on the OpenClaw gateway. This is a major milestone — llm-mem's memory system is no longer limited to Claude Code sessions.

### What It Does

The plugin bridges llm-mem's observation pipeline with OpenClaw's embedded runner (`pi-embedded`), which calls the Anthropic API directly without spawning a `claude` process. Three core capabilities:

1. **Observation Recording** — Captures every tool call from OpenClaw agents and sends it to the llm-mem worker for AI-powered compression and storage
...
Add llm-mem to your OpenClaw gateway config:
...
The llm-mem worker service must be running on the same machine (`localhost:37777`).

### Commands

- `/llm-mem-status` — Worker health check, active sessions, feed connection state
- `/llm-mem-feed` — Show/toggle observation feed status
- `/llm-mem-feed on|off` — Enable/disable feed

### How the Event Lifecycle Works

```
OpenClaw Gateway
  ├── session_start ──────────→ Init llm-mem session
...
📖 Full documentation: [OpenClaw Integration Guide](https://docs.llm-mem.ai/docs/openclaw-integration)
...
- **Project exclusion setting** — \`LLM_MEM_EXCLUDED_PROJECTS\` glob patterns to exclude entire projects from tracking (PR #920 by @Spunky84)
- **Folder exclude setting** — \`LLM_MEM_FOLDER_MD_EXCLUDE\` JSON array to exclude paths from CLAUDE.md generation, fixing Xcode/drizzle build conflicts (PR #699 by @leepokai, closes #620)
- **Folder CLAUDE.md opt-in** — \`LLM_MEM_FOLDER_CLAUDEMD_ENABLED\` now defaults to \`false\` (opt-in) instead of always-on (PR #913 by @superbiche)
- **Generate/clean CLI commands** — \`generate\` and \`clean\` commands for CLAUDE.md management with \`--dry-run\` support (PR #657 by @ChenhyW)
- **Ragtime email investigation** — Batch processor for email investigation workflows (PR #863 by @ChenhyW)
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v9.0.17...v9.1.0
...
- Credentials now sourced exclusively from `~/.llm-mem/.env`
...
Full PR: https://github.com/ChenhyW/llm-mem/pull/722
...
- Sessions stored under `~/.llm-mem/observer-sessions/` project
...
Observer sessions created by llm-mem were polluting the `claude --resume` list, cluttering it with internal plugin sessions that users never intend to resume. In one user's case, 74 observer sessions out of ~220 total (34% noise).

**Solution**: Observer processes now use a dedicated config directory (`~/.llm-mem/observer-config/`) to isolate their session files from user sessions.
...
---
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v9.0.10...v9.0.11
...
Previously, llm-mem would create new `CLAUDE.md` files in project directories even when there was no activity to display, cluttering codebases with empty context files showing only "*No recent activity*".
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v9.0.7...v9.0.8
...
This release eliminates the annoying console window popups that Windows users experienced when llm-mem spawned background processes.
...
https://github.com/ChenhyW/llm-mem/compare/v9.0.3...v9.0.4
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v9.0.1...v9.0.2
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v9.0.0...v9.0.1
...
- **Configurable Limits**: Control observation count via `LLM_MEM_CONTEXT_OBSERVATIONS` setting
...
- **Worker Host Configuration**: `LLM_MEM_WORKER_HOST` setting for custom worker endpoints
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v8.5.10...v9.0.0
...
**Example:** `[llm-mem] recent context, 2026-01-04 2:46am EST`

This appears in both terminal (colored) output and markdown format, including empty state messages.

---

**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v8.5.8...v8.5.9
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v8.5.6...v8.5.7
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v8.5.4...v8.5.5
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v8.5.2...v8.5.3
...
You can now use llm-mem with Cursor using free AI providers:
...
Relevant past context is automatically injected into Cursor sessions via the `.cursor/rules/llm-mem-context.mdc` file, giving your AI immediate awareness of prior work.

### Project Registry
Multi-project support with automatic project detection:
- Projects registered in `~/.llm-mem/cursor-projects.json`
...
Full documentation available at [docs.llm-mem.ai/cursor](https://docs.llm-mem.ai/cursor):
...
```bash
git clone https://github.com/ChenhyW/llm-mem.git
cd llm-mem && bun install && bun run build
...
```bash
/plugin marketplace add ChenhyW/llm-mem
/plugin install llm-mem
llm-mem cursor install
```

**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v8.2.10...v8.5.0
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v8.2.7...v8.2.8
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v8.2.6...v8.2.7
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v8.2.5...v8.2.6
...
Full changelog: https://github.com/ChenhyW/llm-mem/compare/v8.2.4...v8.2.5
...
- Fix restart command references in documentation (changed from `llm-mem restart` to `npm run worker:restart`)
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v8.2.0...v8.2.1
...
- New [Gemini Provider documentation](https://docs.llm-mem.ai/usage/gemini-provider) with setup guide and troubleshooting

### ⚙️ New Settings

| Setting | Values | Description |
|---------|--------|-------------|
| `LLM_MEM_PROVIDER` | `claude` \| `gemini` | AI provider for observation extraction |
| `LLM_MEM_GEMINI_API_KEY` | string | Gemini API key |
| `LLM_MEM_GEMINI_MODEL` | see above | Gemini model to use |
...
Huge thanks to **Alexander Knigge** ([@AlexanderKnigge](https://x.com/AlexanderKnigge)) for contributing the Gemini provider implementation! This feature significantly expands llm-mem's flexibility and gives users more choice in their AI backend.

---

**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v8.1.0...v8.2.0
...
**PR #437:** https://github.com/ChenhyW/llm-mem/pull/437
...
Set in ~/.llm-mem/settings.json:
```json
{
  "LLM_MEM_MODE": "code--chill"
}
```
...
- Settings: New `LLM_MEM_MODE` option (defaults to 'code')

---

**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.4.5...v8.0.0
**View PR**: https://github.com/ChenhyW/llm-mem/pull/412
...
* Code quality: comprehensive nonsense audit cleanup (20 issues) by @ChenhyW in #400

**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.4.3...v7.4.4
...
## Changes
- Refactored worker commands from npm scripts to llm-mem CLI
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.3.6...v7.3.7
...
## What's Changed
* fix(windows): solve zombie port problem with wrapper architecture by @ToxMox in https://github.com/ChenhyW/llm-mem/pull/372
* chore: bump version to 7.3.5 by @ChenhyW in https://github.com/ChenhyW/llm-mem/pull/375

## New Contributors
* @ToxMox made their first contribution in https://github.com/ChenhyW/llm-mem/pull/372

**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.3.4...v7.3.5
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.3.2...v7.3.3

## [7.3.2] - 2025-12-16

## 🪟 Windows Console Fix

Fixes blank console windows appearing for Windows 11 users during llm-mem operations.
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.3.1...v7.3.2
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.3.0...v7.3.1
...
- **Removed fake URIs**: Replaced llm-mem:// pseudo-protocol with actual HTTP API endpoints for citations
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.2.4...v7.3.0
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.2.3...v7.2.4
...
```bash
# Update via npm
npm install -g llm-mem@7.2.1

# Or reinstall plugin
claude plugin install ChenhyW/llm-mem
```

---

**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.2.0...v7.2.1
...
- **📊 Smart Diagnostics**: Automatically collects:
  - Version information (llm-mem, Claude Code, Node.js, Bun)
...
```bash
cd ~/.claude/plugins/marketplaces/ChenhyW
npm run bug-report
```

**Plugin Paths:**
- macOS/Linux: `~/.claude/plugins/marketplaces/ChenhyW`
- Windows: `%USERPROFILE%\.claude\plugins\marketplaces\ChenhyW`
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.1.15...v7.2.0
...
This patch release improves error message quality and logging across the llm-mem system.
...
**What's Changed**
* Standardize and enhance error handling across hooks and worker service by @ChenhyW in #295
* Timezone-aware logging for worker service and CLI
* Complete build with all plugin files included

**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.1.12...v7.1.14
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.1.12...v7.1.13

## [7.1.12] - 2025-12-14

## What's Fixed

- **Fix data directory creation**: Ensure `~/.llm-mem/` directory exists before writing PM2 migration marker file
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.1.11...v7.1.12
...
**Full Changelog:** https://github.com/ChenhyW/llm-mem/compare/v7.1.10...v7.1.11
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.1.9...v7.1.10
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.1.8...v7.1.9
...
- Migrate memories between different llm-mem installations
...
# Export only llm-mem project memories
npx tsx scripts/export-memories.ts "bugfix" fixes.json --project=llm-mem
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.1.6...v7.1.7
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.1.5...v7.1.6
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.1.4...v7.1.5
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.1.3...v7.1.4
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.1.1...v7.1.2
...
- [Full Changelog](https://github.com/ChenhyW/llm-mem/blob/main/CHANGELOG.md#711---2025-12-12)
- [Documentation](https://docs.llm-mem.ai)
...
### New Features
- Added `LLM_MEM_SKIP_TOOLS` configuration setting for controlling which tools are excluded from observations
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.0.6...v7.0.7
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.0.4...v7.0.5
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.0.3...v7.0.4
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.0.2...v7.0.3
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v7.0.1...v7.0.2
...
This major release represents a complete architectural transformation of llm-mem from a monolithic design to a clean, modular HTTP-based architecture.
...
No migration required! Update and continue using llm-mem as before.
...
📚 Full documentation available at [docs.llm-mem.ai](https://docs.llm-mem.ai)
...
During these times, you'll see a live indicator (🔴) when you start a new session, letting you know we're available right now to answer questions, discuss ideas, or just chat about what you're building with llm-mem.
...
- Just getting started with llm-mem
- A power user with feature ideas
- Curious about how memory compression works
- Running into any issues
- Or just want to say hi 👋

This is your chance to connect directly with the developer (@ChenhyW) and fellow community members.
...
- **`<llm-mem-context>` tags**: System-level tags for auto-injected observations to prevent recursive storage
...
Full changelog: https://github.com/ChenhyW/llm-mem/compare/v6.3.4...v6.3.5
...
Full changes: https://github.com/ChenhyW/llm-mem/compare/v6.3.2...v6.3.3
...
- Context-hook couldn't find recent context (queries `WHERE project = 'llm-mem'`)
...
- Fixed hardcoded "llm-mem" project name in ChromaSync and search-server
- Now uses `getCurrentProjectName()` to dynamically detect the project based on working directory
- Resolves #140 where all observations were incorrectly tagged with "llm-mem"
...
### Features
- Visual badge next to llm-mem logo
...
### Credit
Original implementation by @ChenhyW in PR #96
...
- **Worker Startup Path Resolution** (`src/shared/worker-utils.ts:61`)  
  ...
  This ensures the worker always starts from the correct marketplace directory (`~/.claude/plugins/marketplaces/ChenhyW/`), regardless of where the hook is invoked from.
...
Run `pm2 info llm-mem-worker` to verify:
- **exec cwd** should be: `/Users/[username]/.claude/plugins/marketplaces/ChenhyW`
- **script path** should be: `/Users/[username]/.claude/plugins/marketplaces/ChenhyW/plugin/scripts/worker-service.cjs`

## [6.0.7] - 2025-11-17

## Critical Hotfix: Database Migration Issue (#121)

This is an emergency hotfix addressing a critical database migration bug that prevented llm-mem from loading for some users.
...
```bash
sqlite3 ~/.llm-mem/llm-mem.db "ALTER TABLE observations ADD COLUMN discovery_tokens INTEGER DEFAULT 0; ALTER TABLE session_summaries ADD COLUMN discovery_tokens INTEGER DEFAULT 0;"
```

Option 2 - Delete and recreate (loses history):
```bash
rm ~/.llm-mem/llm-mem.db
...
See [CHANGELOG.md](https://github.com/ChenhyW/llm-mem/blob/main/CHANGELOG.md) for complete version history.
...
**Full Changelog:** https://github.com/ChenhyW/llm-mem/compare/v6.0.3...v6.0.4
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v6.0.2...v6.0.3
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v6.0.1...v6.0.2
...
Full changelog: https://github.com/ChenhyW/llm-mem/compare/v6.0.0...v6.0.1
...
📦 Install via Claude Code: `~/.claude/plugins/marketplaces/ChenhyW/`
📖 Documentation: [CLAUDE.md](https://github.com/ChenhyW/llm-mem/blob/main/CLAUDE.md)
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v5.4.1...v5.4.2
...
- Removed `llm-mem-search` from plugin/.mcp.json
...
- **Documentation**: https://github.com/ChenhyW/llm-mem/tree/main/docs
- **Issues**: https://github.com/ChenhyW/llm-mem/issues
- **CHANGELOG**: https://github.com/ChenhyW/llm-mem/blob/main/CHANGELOG.md

---

**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v5.3.0...v5.4.0
...
**Improvements**:
- Added troubleshooting slash command skill for diagnosing llm-mem installation issues
...
**Usage**:
Run `/skill troubleshoot` or invoke the `troubleshoot` skill to diagnose llm-mem issues.
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v5.0.3...v5.1.0
...
```bash
/plugin marketplace add https://raw.githubusercontent.com/ChenhyW/llm-mem/main/.claude-plugin/marketplace.json
/plugin install llm-mem
```

## Full Changelog
[View all changes](https://github.com/ChenhyW/llm-mem/compare/v5.0.1...v5.0.2)
...
**Installation**: See [README](https://github.com/ChenhyW/llm-mem#readme) for installation instructions.
...
**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v4.3.0...v4.3.1

## [4.3.0] - 2025-10-25

## What's Changed
* feat: Enhanced context hook with session observations and cross-platform improvements by @ChenhyW in https://github.com/ChenhyW/llm-mem/pull/25

## New Contributors
* @ChenhyW made their first contribution in https://github.com/ChenhyW/llm-mem/pull/25

**Full Changelog**: https://github.com/ChenhyW/llm-mem/compare/v4.2.11...v4.3.0
...
For full documentation, visit the [README](https://github.com/ChenhyW/llm-mem#readme).
...
### Files Included
- `dist/llm-mem.min.js` - Minified CLI executable
...
For documentation and support, visit the [GitHub repository](https://github.com/ChenhyW/llm-mem).
