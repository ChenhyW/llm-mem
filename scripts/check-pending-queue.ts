...
const DEFAULT_WORKER_HOST = workerSettings.LLM_MEM_WORKER_HOST;
const DEFAULT_WORKER_PORT = workerSettings.LLM_MEM_WORKER_PORT;

function resolveWorkerHost(): string {
  return process.env.LLM_MEM_WORKER_HOST || DEFAULT_WORKER_HOST;
}

function resolveWorkerPort(): string {
  const raw = process.env.LLM_MEM_WORKER_PORT;
  if (raw === undefined || raw === '') return DEFAULT_WORKER_PORT;
  const parsed = parseInt(raw, 10);
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > 65535) {
    console.warn(
      `[check-pending-queue] Invalid LLM_MEM_WORKER_PORT=${JSON.stringify(raw)}; ` +
...
Environment:
  LLM_MEM_WORKER_HOST  Worker host (default: ${DEFAULT_WORKER_HOST})
  LLM_MEM_WORKER_PORT  Worker port (default: ${DEFAULT_WORKER_PORT})
...
What is this for?
  If the llm-mem worker has unprocessed observations queued, this script
...
`);
  ...
  if (!healthy) {
    ...
    console.log('  cd ~/.claude/plugins/marketplaces/ChenhyW-llm-mem && npm run worker:start\n');
    process.exit(1);
  }
