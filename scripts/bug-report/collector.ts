...
async function getDatabaseInfo(
  ...
  try {
    const dbPath = path.join(dataDir, "llm-mem.db");
...
export async function collectDiagnostics(
  ...
  const homeDir = os.homedir();
  const dataDir = path.join(homeDir, ".llm-mem");
  ...
  const workerHost = process.env.LLM_MEM_WORKER_HOST || workerSettings.LLM_MEM_WORKER_HOST;
  const configuredWorkerPort = process.env.LLM_MEM_WORKER_PORT || workerSettings.LLM_MEM_WORKER_PORT;
  ...
  const database = {
    path: sanitizePath(path.join(dataDir, "llm-mem.db")),
