...
const WORKER_HOST = process.env.LLM_MEM_WORKER_HOST || workerSettings.LLM_MEM_WORKER_HOST;
const WORKER_PORT = process.env.LLM_MEM_WORKER_PORT || workerSettings.LLM_MEM_WORKER_PORT;
...
async function importMemories(inputFile: string) {
  ...
  try {
    ...
    console.error('   Please ensure the llm-mem worker is running.');
    process.exit(1);
  }
