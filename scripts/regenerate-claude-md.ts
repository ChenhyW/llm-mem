...
const DB_PATH = path.join(os.homedir(), '.llm-mem', 'llm-mem.db');
const SETTINGS_PATH = path.join(os.homedir(), '.llm-mem', 'settings.json');
const settings = SettingsDefaultsManager.loadFromFile(SETTINGS_PATH);
const OBSERVATION_LIMIT = parseInt(settings.LLM_MEM_CONTEXT_OBSERVATIONS, 10) || 50;
...
function cleanupAutoGeneratedFiles(workingDir: string, dryRun: boolean): void {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            if (content.includes('<llm-mem-context>')) {
  ...
  for (const file of filesToProcess) {
    ...
    try {
      ...
      const stripped = content.replace(/<llm-mem-context>[\s\S]*?<\/llm-mem-context>/g, '').trim();
