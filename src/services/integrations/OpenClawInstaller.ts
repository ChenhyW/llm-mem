...
function registerPluginInOpenClawConfig(
  ...
  config.plugins.slots.memory = 'llm-mem';

  if (!config.plugins.entries['llm-mem']) {
    config.plugins.entries['llm-mem'] = {
      enabled: true,
      config: {
        workerPort,
        project,
    ...
    config.plugins.entries['llm-mem'].enabled = true;
    if (!config.plugins.entries['llm-mem'].config) {
      config.plugins.entries['llm-mem'].config = {};
    }
    const existingPluginConfig = config.plugins.entries['llm-mem'].config;
...
function unregisterPluginFromOpenClawConfig(): void {
  ...
  if (config.plugins?.entries?.['llm-mem']) {
    delete config.plugins.entries['llm-mem'];
  }

  if (config.plugins?.slots?.memory === 'llm-mem') {
...
function copyPluginFilesAndRegister(
  ...
  const workerPort = SettingsDefaultsManager.getInt('LLM_MEM_WORKER_PORT');
...
export function checkOpenClawStatus(): number {
  ...
  if (existsSync(configFilePath)) {
    const config = readOpenClawConfig();
    const isRegistered = config.plugins?.entries?.['llm-mem'] !== undefined;
    const isEnabled = config.plugins?.entries?.['llm-mem']?.enabled === true;
    const isMemorySlot = config.plugins?.slots?.memory === 'llm-mem';
    ...
    if (isRegistered) {
      const pluginConfig = config.plugins.entries['llm-mem'].config;
      if (pluginConfig) {
