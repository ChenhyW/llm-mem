 ...
 *    install day.
 *
 * Idempotency: a completion marker (~/.llm-mem/backfill.json) is the
...
/**
 * Predates llm-mem's first release. Rows whose normalized epoch falls
 ...
 *  3. build events
 *  4. LLM_MEM_TELEMETRY_DEBUG=1   -> stderr dry-run, NO send, NO marker
...
async function executeHistoricalBackfill(db: Database): Promise<void> {
  ...
  if (process.env.LLM_MEM_TELEMETRY_DEBUG === '1') {
