 ...
 * 2. LLM_MEM_TELEMETRY env: '0'/'false'/'off' -> off, '1'/'true'/'on' -> on
 * 3. telemetry.json config: enabled === true -> on, enabled === false -> off
 * 4. Default: on (opt-out — anonymous events only; see docs.llm-mem.ai/telemetry)
 */
export function explainTelemetryConsent(
  ...
  const override = env.LLM_MEM_TELEMETRY?.toLowerCase();
 ...
 *
 * `LLM_MEM_TELEMETRY_ERRORS=0` (or 'false'/'off') disables real exception
...
export function isErrorTelemetryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.LLM_MEM_TELEMETRY_ERRORS?.toLowerCase();
