...
export type CloudSyncSettingKeys = Pick<SettingsDefaults,
  ...
  | 'LLM_MEM_CLOUD_SYNC_HUB_URL'
  | 'LLM_MEM_CLOUD_SYNC_DEVICE_ID'
  | 'LLM_MEM_CLOUD_SYNC_DEVICE_NAME'
>;
...
export class CloudSync {
  ...
  constructor(db: Database, settings: CloudSyncSettingKeys, options: CloudSyncOptions = {}) {
    this.db = db;
    this.token = settings.LLM_MEM_CLOUD_SYNC_TOKEN ?? '';
    this.userId = settings.LLM_MEM_CLOUD_SYNC_USER_ID ?? '';
    // Launch contract: the Hub URL has no default. Empty means sync is off;
    // there is no application-API or per-kind fallback lane.
    this.hubUrl = (settings.LLM_MEM_CLOUD_SYNC_HUB_URL ?? '').trim().replace(/\/+$/, '');
    // Human-readable device label for the dashboard's Devices panel.
    this.deviceName = (settings.LLM_MEM_CLOUD_SYNC_DEVICE_NAME || hostname() || '').slice(0, 80);
    ...
    if (this.isConfigured()) {
      this.deviceId = this.resolveDeviceId(settings.LLM_MEM_CLOUD_SYNC_DEVICE_ID ?? '');
    }
  }
  ...
  private persistDeviceId(deviceId: string): void {
    ...
    const target = settings.env && typeof settings.env === 'object'
      ...
      : settings;
    target.LLM_MEM_CLOUD_SYNC_DEVICE_ID = deviceId;
