...
// (LLM_MEM_CLOUD_SYNC_WS, default on) the client also holds one Bun-native
...
export interface SyncClientOptions {
  /** Sync hub base URL (LLM_MEM_CLOUD_SYNC_HUB_URL). */
  hubUrl: string;
  token: string;
  userId: string;
  /** MUST be the CloudSync-resolved device id (single identity source). */
  deviceId: string;
  /** Human-readable Hub device label (LLM_MEM_CLOUD_SYNC_DEVICE_NAME). */
  deviceName?: string;
  ...
  now?: () => number;
  /**
   * Advisory WebSocket gate (LLM_MEM_CLOUD_SYNC_WS ≠ 'false'). Defaults to
...
export class SyncClient {
  ...
  constructor(apply: SyncApply, options: SyncClientOptions) {
    ...
    if (!hubUrl) {
      throw new Error('SyncClient requires a non-empty hubUrl (LLM_MEM_CLOUD_SYNC_HUB_URL)');
    }
    if (!options.deviceId) {
