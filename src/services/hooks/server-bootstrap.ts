...
//
// Bootstrapping flow:
//   1. Connect to Postgres (LLM_MEM_SERVER_DATABASE_URL).
...
//   4. Persist the plaintext key to ~/.llm-mem/settings.json under
//      `LLM_MEM_SERVER_API_KEY` (the new canonical key after the
//      server-beta → server rename). Reads in `runtime-selector.ts`
//      dual-accept the legacy `LLM_MEM_SERVER_BETA_*` keys, but writes
...
export function persistServerSettings(
  ...
  // `LLM_MEM_SERVER_BETA_*` keys are dual-accepted by reads in
  ...
  flat.LLM_MEM_SERVER_API_KEY = values.apiKey;
  flat.LLM_MEM_SERVER_PROJECT_ID = values.projectId;
  if (values.serverBaseUrl) {
    flat.LLM_MEM_SERVER_URL = values.serverBaseUrl;
  }
...
function buildPoolFromEnv(): PostgresPool {
  ...
  if (!config) {
    throw new Error(
      'Cannot bootstrap server API key: LLM_MEM_SERVER_DATABASE_URL is not set.',
    );
  }
