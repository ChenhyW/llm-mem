...
//
// Reads `LLM_MEM_RUNTIME` from `~/.llm-mem/settings.json` (via
...
// existing settings.json files and `LLM_MEM_RUNTIME` values keep
// working. Likewise, new settings keys `LLM_MEM_SERVER_{URL,API_KEY,
// PROJECT_ID}` are read first and fall back to the legacy
// `LLM_MEM_SERVER_BETA_*` keys when unset.
...
export function selectRuntime(): SelectedRuntime {
  ...
  const raw = (settings.LLM_MEM_RUNTIME ?? 'worker').trim().toLowerCase();
...
export function buildServerContext(): ServerRuntimeContext | null {
  ...
  const serverBaseUrl = pickFirstNonEmpty(
    settings.LLM_MEM_SERVER_URL,
    settings.LLM_MEM_SERVER_BETA_URL,
  );
  const apiKey = pickFirstNonEmpty(
    settings.LLM_MEM_SERVER_API_KEY,
    settings.LLM_MEM_SERVER_BETA_API_KEY,
  );
  const projectId = pickFirstNonEmpty(
    settings.LLM_MEM_SERVER_PROJECT_ID,
    settings.LLM_MEM_SERVER_BETA_PROJECT_ID,
  );
