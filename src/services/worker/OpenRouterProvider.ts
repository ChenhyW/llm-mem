 ...
 *
 * The endpoint is resolved from LLM_MEM_OPENROUTER_BASE_URL (settings or env;
 ...
 * LLM_MEM_OPENROUTER_MODEL. See src/shared/openrouter-base-url.ts for the
...
export class OpenRouterProvider extends OpenAICompatibleProvider<OpenRouterConfig> {
  ...
  protected missingApiKeyError(): Error {
    return new Error('OpenRouter API key not configured. Set LLM_MEM_OPENROUTER_API_KEY in settings or OPENROUTER_API_KEY environment variable.');
  }
  ...
  private getOpenRouterConfig(): OpenRouterConfig {
    ...
    const apiKey = settings.LLM_MEM_OPENROUTER_API_KEY || getCredential('OPENROUTER_API_KEY') || '';
    ...
    const rawModel: unknown = settings.LLM_MEM_OPENROUTER_MODEL;
    ...
    const baseUrl = settings.LLM_MEM_OPENROUTER_BASE_URL || process.env.OPENROUTER_BASE_URL || '';
    const apiUrl = resolveOpenRouterChatCompletionsUrl(baseUrl);

    const siteUrl = settings.LLM_MEM_OPENROUTER_SITE_URL || '';
    const appName = settings.LLM_MEM_OPENROUTER_APP_NAME || 'llm-mem';
...
export function isOpenRouterAvailable(): boolean {
  const settingsPath = USER_SETTINGS_PATH;
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return !!(settings.LLM_MEM_OPENROUTER_API_KEY || getCredential('OPENROUTER_API_KEY'));
}

export function isOpenRouterSelected(): boolean {
  const settingsPath = USER_SETTINGS_PATH;
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return settings.LLM_MEM_PROVIDER === 'openrouter';
}
