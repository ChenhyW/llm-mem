 ...
 * ~/.llm-mem/.env, see #2357) is environmental — re-logging it on every
...
export class ClaudeProvider {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    ...
    const maxConcurrent = parseInt(settings.LLM_MEM_MAX_CONCURRENT_AGENTS, 10) || 2;
    ...
    try {
      ...
      const queryResult = query({
        ...
        options: buildHardenedSdkOptions({
          ...
          model: modelId,
          env: isolatedEnv,  // Use isolated credentials from ~/.llm-mem/.env, not process.env
      ...
      for await (const message of queryResult) {
        ...
        if (message.type === 'assistant') {
          ...
          if (typeof textContent === 'string' && textContent.includes('Invalid API key')) {
            throw new Error('Invalid API key: check your API key configuration in ~/.llm-mem/settings.json or ~/.llm-mem/.env');
          }
  ...
  private getModelId(): string {
    ...
    return resolveTierAlias(settings.LLM_MEM_MODEL, settings);
  }
}
