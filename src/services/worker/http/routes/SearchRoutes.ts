...
// enough that toggling LLM_MEM_WELCOME_HINT_ENABLED is responsive in
...
export class SearchRoutes extends BaseRouteHandler {
  ...
  private handleContextInject = this.wrapHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    // settings cache TTL.
    const hintEnabledRaw = process.env.LLM_MEM_WELCOME_HINT_ENABLED ?? settings.LLM_MEM_WELCOME_HINT_ENABLED;
    ...
    if (hintEnabled && !full) {
      ...
      if (!this.projectsHaveObservations(sessionStore, projects, platformSource)) {
        const port = process.env.LLM_MEM_WORKER_PORT ?? settings.LLM_MEM_WORKER_PORT;
    ...
    if (contextResult.stats) {
      ...
      telemetryBuffer.record('context_injected', null, {
        outcome: 'ok',
        duration_ms: Date.now() - injectStartedAt,
        mode: settingsSnapshot.LLM_MEM_MODE,
        provider: settingsSnapshot.LLM_MEM_PROVIDER,
        ...contextResult.stats,
      });
    }
