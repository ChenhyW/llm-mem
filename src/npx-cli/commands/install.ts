...
function registerMarketplace(): void {
  ...
  knownMarketplaces['ChenhyW'] = {
    source: {
      source: 'github',
      repo: 'ChenhyW/llm-mem',
    },
...
function registerPlugin(version: string): void {
  ...
  installedPlugins.plugins['llm-mem@ChenhyW'] = [
    {
      scope: 'user',
...
function enablePluginInClaudeSettings(): void {
  ...
  settings.enabledPlugins['llm-mem@ChenhyW'] = true;
...
function readRawStoredAuthMethod(): 'subscription' | 'api-key' | 'gateway' | undefined {
  try {
    const value = readFlatSettings(USER_SETTINGS_PATH)?.LLM_MEM_CLAUDE_AUTH_METHOD;
...
async function promptProvider(options: InstallOptions): Promise<ProviderId> {
  const initialProvider = (getSetting('LLM_MEM_PROVIDER') as ProviderId) || 'claude';
  ...
  const persistClaudeProvider = (authMethod?: 'subscription' | 'api-key' | 'gateway') => {
    ...
    const wrote = mergeSettings({
      ...
      LLM_MEM_CLAUDE_AUTH_METHOD: resolvedAuthMethod,
    });
    if (wrote) log.info('Saved Claude Agent SDK configuration to ~/.llm-mem/settings.json');
  };
  ...
  const configureGateway = async (): Promise<void> => {
    ...
    if (tokenCancelled || tokenInput.length === 0) {
      ...
      log.info('Configured Claude Agent SDK gateway in ~/.llm-mem/.env.');
    }
  };
  ...
  if (!isInteractive) {
    if (options.provider) {
      ...
      const wrote = mergeSettings({ LLM_MEM_PROVIDER: options.provider });
      if (wrote) log.info(`Saved provider=${options.provider} to ~/.llm-mem/settings.json`);
      log.warn(`Provider=${options.provider} requested non-interactively. API key prompt skipped — set LLM_MEM_${options.provider.toUpperCase()}_API_KEY and LLM_MEM_PROVIDER in settings.json or env manually if not already set.`);
  ...
  if (existingKey && existingKey.trim().length > 0) {
    const wrote = mergeSettings({ LLM_MEM_PROVIDER: selectedProvider });
    if (wrote) log.info(`Saved provider=${selectedProvider} to ~/.llm-mem/settings.json`);
  ...
  const wrote = mergeSettings({
    LLM_MEM_PROVIDER: selectedProvider,
    [keyEnvName]: apiKey,
  });
  if (wrote) {
    log.info(`Saved provider=${selectedProvider} to ~/.llm-mem/settings.json`);
  }
  return selectedProvider;
}
...
async function promptClaudeModel(options: InstallOptions): Promise<void> {
  ...
  if (options.model && !allowCustomModel) {
    ...
    const wrote = mergeSettings({ LLM_MEM_MODEL: options.model });
    if (wrote) {
      log.info(`Saved Claude model=${options.model} to ~/.llm-mem/settings.json`);
    }
    return;
  }
  if (options.model && allowCustomModel) {
    const wrote = mergeSettings({ LLM_MEM_MODEL: options.model });
    if (wrote) {
      log.info(`Saved gateway model=${options.model} to ~/.llm-mem/settings.json`);
    }
    return;
  }
  ...
  const initialModel = getSetting('LLM_MEM_MODEL');
  ...
  if (allowCustomModel) {
    ...
    const wrote = mergeSettings({ LLM_MEM_MODEL: selectedModel });
    if (wrote) {
      log.info(`Saved gateway model=${selectedModel} to ~/.llm-mem/settings.json`);
    }
    return;
  }
  ...
  const wrote = mergeSettings({ LLM_MEM_MODEL: selectedModel });
  if (wrote) {
    log.info(`Saved Claude model=${selectedModel} to ~/.llm-mem/settings.json`);
  }
}
...
// and the confirmation email server-side. LLM_MEM_SIGNUP_URL overrides the
...
const SIGNUP_ENDPOINT = process.env.LLM_MEM_SIGNUP_URL?.trim() || DEFAULT_SIGNUP_ENDPOINT;
...
function parseStoredSignup(): StoredSignup | null {
  ...
  if (!flat) return null;
  const email = typeof flat.LLM_MEM_ONLINE_SIGNUP_EMAIL === 'string' ? flat.LLM_MEM_ONLINE_SIGNUP_EMAIL : '';
  if (!email) return null;
  return {
    email,
    note: typeof flat.LLM_MEM_ONLINE_SIGNUP_NOTE === 'string' ? flat.LLM_MEM_ONLINE_SIGNUP_NOTE : '',
    sent: flat.LLM_MEM_ONLINE_SIGNUP_SENT === 'true',
  };
}
...
async function promptTelemetryOptIn(): Promise<void> {
  ...
  p.log.message(styleText('dim', 
    ...
    + 'Details: https://docs.llm-mem.ai/telemetry · Change anytime: llm-mem telemetry disable',
  ));
...
async function promptCmemOnlineOptIn(version: string): Promise<void> {
  ...
  if (!isInteractive) return;
  if (process.env.CI) return;
  if (String(process.env.LLM_MEM_ONLINE_OPTIN ?? '').trim().toLowerCase() === 'false') return;
  ...
  if (prior) {
    ...
    if (!prior.sent) {
      ...
      if (ok) mergeSettings({ LLM_MEM_ONLINE_SIGNUP_SENT: 'true' });
    }
    return;
  }
  ...
  mergeSettings({
    LLM_MEM_ONLINE_SIGNUP_EMAIL: email,
    LLM_MEM_ONLINE_SIGNUP_NOTE: note,
    LLM_MEM_ONLINE_SIGNUP_AT: new Date().toISOString(),
    LLM_MEM_ONLINE_SIGNUP_SENT: ok ? 'true' : 'false',
  });
  if (ok) {
...
async function runInstallCommandInner(options: InstallOptions, summary: InstallSummary): Promise<void> {
  ...
  const segments = [`${styleText('bold', 'llm-mem')} ${styleText('cyan', `v${version}`)}`];
  ...
  {
    if (needsMarketplace) {
      const installPort = getSetting('LLM_MEM_WORKER_PORT');
  ...
  // via --disable-auto-memory. llm-mem's hook-based memory is the intended
  ...
  await runTasks([
    {
      ...
      task: async (message) => {
        ...
        const port = Number(getSetting('LLM_MEM_WORKER_PORT'));
  ...
  const workerHost = getSetting('LLM_MEM_WORKER_HOST');
  const workerUrlHost = formatHostForUrl(workerHost);
  const workerPort = getSetting('LLM_MEM_WORKER_PORT');
  ...
  const nextSteps = [
    ...
    `Everything stays in ${styleText('cyan', '~/.llm-mem')} on this machine.`,
    ``,
    `${styleText('dim', 'How it works: /how-it-works   ·   Disable first-session hint: LLM_MEM_WELCOME_HINT_ENABLED=false')}`,
    `${styleText('dim', 'Note: close all Claude Code sessions before uninstalling, or ~/.llm-mem will be recreated by active hooks.')}`,
  ];
