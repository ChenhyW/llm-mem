/**
 * `npx llm-mem telemetry [status|enable|disable]` — manage anonymous usage
 ...
 * `telemetry disable`, LLM_MEM_TELEMETRY=0, or DO_NOT_TRACK=1.
 *
 * Full privacy documentation: https://docs.llm-mem.ai/telemetry
 */
...
const SOURCE_LABELS: Record<TelemetryConsentSource, string> = {
  ...
  env: 'LLM_MEM_TELEMETRY environment variable',
  config: 'telemetry.json config file',
  default: 'default (on — no opt-out recorded)',
};

function printTelemetryUsage(): void {
  console.error(`Usage: ${styleText('bold', 'npx llm-mem telemetry [status|enable|disable]')}`);
...
async function runTelemetryEnable(): Promise<void> {
  ...
  p.intro(styleText(['bgBlue', 'white'], ' llm-mem telemetry '));
  ...
  p.outro(`Change your mind anytime: ${styleText('cyan', 'npx llm-mem telemetry disable')}`);
}
