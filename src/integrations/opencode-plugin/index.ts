 ...
 * object whose keys are OpenCode's real hook names. The hooks llm-mem binds
 ...
 * `event.type`. The only bus event types llm-mem reacts to are
...
function resolveWorkerPort(): string {
  // Canonical resolution: LLM_MEM_WORKER_PORT env override, else the
  // UID-derived default — identical to the rest of the codebase (#2406).
  return SettingsDefaultsManager.get("LLM_MEM_WORKER_PORT");
}

function resolveWorkerHost(): string {
  return SettingsDefaultsManager.get("LLM_MEM_WORKER_HOST");
}
...
function workerPostFireAndForget(
  ...
  fetch(`${WORKER_BASE_URL}${path}`, {
    ...
    if (!message.includes("ECONNREFUSED")) {
      console.warn(`[llm-mem] Worker POST ${path} failed: ${message}`);
    }
  });
}
...
async function workerGetText(path: string): Promise<string | null> {
  try {
    ...
    if (!response.ok) {
      console.warn(`[llm-mem] Worker GET ${path} returned ${response.status}`);
      return null;
    }
    ...
    const message = error instanceof Error ? error.message : String(error);
    if (!message.includes("ECONNREFUSED")) {
      console.warn(`[llm-mem] Worker GET ${path} failed: ${message}`);
    }
    return null;
  }
}
    ...
    tool: {
      llm_mem_search: {
        ...
        async execute(args: Record<string, unknown>): Promise<string> {
          ...
          if (!text) {
            return "llm-mem worker is not running. Start it with: npx llm-mem start";
          }
...
export function parseSearchResponse(text: string, query: string): string {
  ...
  try {
    ...
    console.warn(
      "[llm-mem] Failed to parse search results:",
