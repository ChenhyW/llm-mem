...
async function summarizeViaServer(_runtime: ServerRuntimeContext, _sessionId: string, _lastAssistantMessage: string, _platformSource: string): Promise<HookResult> {
  // Server mode removed; this dead path should never be reached now.
  logger.warn('HOOK', 'summarizeViaServer called in post-server-mode build (unreachable)');
