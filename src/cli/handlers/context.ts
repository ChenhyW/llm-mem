...
export const contextHandler: EventHandler = {
  async execute(input: NormalizedHookInput): Promise<HookResult> {
    ...
    // Honor LLM_MEM_EXCLUDED_PROJECTS on the inject/read path too. The
    ...
    const showTerminalOutput =
      settings.LLM_MEM_CONTEXT_SHOW_TERMINAL_OUTPUT === 'true'
    ...
    if (staleReason) {
      const hint = `[llm-mem] Claude Desktop OAuth token is stale: ${staleReason}\nPlease re-login via Claude Desktop to refresh the token.`;
