...
// settings) so the user can flip LLM_MEM_RUNTIME without restarting
// the MCP server.
...
function requireServerForObservationTool(toolName: string): ServerAvailable {
  ...
  if (!resolution) {
    throw new ServerClientError(
      'transport',
      `${toolName} requires LLM_MEM_RUNTIME=server. Current runtime is "worker"; use the existing search/timeline/get_observations tools for worker-mode memory access.`,
    );
  }
...
**Why:** 10x token savings. Never fetch full details without filtering first.`
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        project: { type: 'string', description: 'Project name, e.g. llm-mem/night-parsnip' },
        projects: {
          oneOf: [
  ...
  // Phase 8 — observation_* tools backed by server REST core.
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        projectId: { type: 'string', description: 'Project id (falls back to LLM_MEM_SERVER_PROJECT_ID)' },
...
async function main() {
  ...
  setTimeout(async () => {
    // Phase 8 — when LLM_MEM_RUNTIME=server (or legacy `server-beta`,
