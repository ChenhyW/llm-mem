...
// temp file via LLM_MEM_ENV_FILE without depending on module-load order.
// Production callers see the canonical ~/.llm-mem/.env path through
// paths.envFile() unchanged.
export function envFilePath(): string {
  return process.env.LLM_MEM_ENV_FILE ?? paths.envFile();
}
...
const BLOCKED_ENV_VARS = [
  ...
  'ANTHROPIC_AUTH_TOKEN',    // Same leak risk as ANTHROPIC_API_KEY; a token inherited from the
                             ...
                             // The fresh token from ~/.llm-mem/.env is re-injected below
  ...
  'ANTHROPIC_BASE_URL',      // Issue #2375: same leak class as AUTH_TOKEN. A leaked BASE_URL
                             ...
                             // Re-injected from ~/.llm-mem/.env when configured.
...
/**
 * The only env keys ever copied out of ~/.llm-mem/.env. This is the
...
function serializeEnvFile(env: Record<string, string>): string {
  const lines: string[] = [
    '# llm-mem credentials',
    '# This file stores keys and gateway settings for the llm-mem memory agent',
    '# Edit this file or use llm-mem settings to configure',
    '',
  ];
 ...
 * are populated ONLY from ~/.llm-mem/.env — never from the parent shell.
...
export async function buildIsolatedEnvWithFreshOAuth(
  ...
  // gateway-appropriate token in ~/.llm-mem/.env if their gateway requires
...
export function getAuthMethodDescription(): string {
  if (hasAnthropicApiKey()) {
    return 'API key (from ~/.llm-mem/.env)';
  }
  if (hasAnthropicAuthToken()) {
    return 'Gateway auth token (from ~/.llm-mem/.env)';
  }
