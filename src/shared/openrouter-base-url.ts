 ...
 * to turn the optional LLM_MEM_OPENROUTER_BASE_URL setting into a concrete
 * `/chat/completions` endpoint. This makes the OpenRouter client a generic
 * OpenAI-compatible client.
 *
 * Closes #2382 (LLM_MEM_OPENROUTER_BASE_URL), #2590 (custom provider with
 ...
 * LLM_MEM_OPENROUTER_MODEL setting (passed verbatim), a user points the base
 * URL at any OpenAI-compatible endpoint and selects an arbitrary model id.
 *
 * Usage examples (set LLM_MEM_PROVIDER=openrouter, then):
 *
 *   DeepSeek:
 *     LLM_MEM_OPENROUTER_BASE_URL = https://api.deepseek.com
 *     LLM_MEM_OPENROUTER_MODEL    = deepseek-chat
 *     OPENROUTER_API_KEY (in ~/.llm-mem/.env) = <deepseek key>
 *
 *   LM Studio (local, no key required):
 *     LLM_MEM_OPENROUTER_BASE_URL = http://localhost:1234/v1
 *     LLM_MEM_OPENROUTER_MODEL    = <any local model id>
 *
 *   Generic OpenAI-compatible endpoint:
 *     LLM_MEM_OPENROUTER_BASE_URL = https://my-gateway.example.com/v1
 *     LLM_MEM_OPENROUTER_MODEL    = <model id>
 */
