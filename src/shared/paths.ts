...
export function resolveDataDir(): string {
  if (process.env.LLM_MEM_DATA_DIR) {
    return expandHome(process.env.LLM_MEM_DATA_DIR);
  }

  const defaultDataDir = join(homedir(), '.llm-mem');
  ...
  try {
    if (existsSync(settingsPath)) {
      ...
      if (settings.LLM_MEM_DATA_DIR) {
        return expandHome(settings.LLM_MEM_DATA_DIR);
      }
    }
  } catch {
...
export const MARKETPLACE_ROOT = join(CLAUDE_CONFIG_DIR, 'plugins', 'marketplaces', 'ChenhyW-llm-mem');

export const LOGS_DIR = join(DATA_DIR, 'logs');
export const USER_SETTINGS_PATH = join(DATA_DIR, 'settings.json');
export const DB_PATH = join(DATA_DIR, 'llm-mem.db');
...
export const paths = {
  ...
  database: () => join(DATA_DIR, 'llm-mem.db'),
