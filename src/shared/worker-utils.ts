...
function getWorkerSettingsPath(): string {
  return path.join(SettingsDefaultsManager.get('LLM_MEM_DATA_DIR'), 'settings.json');
}
...
export function getWorkerPort(): number {
  ...
  cachedPort = parseInt(settings.LLM_MEM_WORKER_PORT, 10);
  return cachedPort;
}
...
export function getWorkerHost(): string {
  ...
  const settings = getWorkerSettings();
  cachedHost = settings.LLM_MEM_WORKER_HOST;
  return cachedHost;
}
...
export function cacheWorkerScriptCandidates(
  cacheRoot: string = path.join(path.dirname(path.dirname(MARKETPLACE_ROOT)), 'cache', 'ChenhyW', 'llm-mem')
...
export function resolveWorkerScript(): WorkerScriptCandidate | null {
  const override = process.env.LLM_MEM_WORKER_SCRIPT_PATH?.trim();
  if (override) {
    if (existsSync(override)) return { scriptPath: override, version: null };
    logger.debug('SYSTEM', 'Ignoring missing LLM_MEM_WORKER_SCRIPT_PATH override', { override });
  }
...
export async function ensureWorkerRunning(): Promise<boolean> {
  ...
  if (await isWorkerPortAlive()) {
    ...
    if (stalePidInfo === null || stalePidInfo.port !== getWorkerPort()) {
      logger.error('SYSTEM', 'Stale worker is serving the port but the PID file does not identify it; kill the llm-mem worker process manually', {
        port: getWorkerPort(),
...
function getFailLoudThreshold(): number {
  try {
    ...
    const raw = settings.LLM_MEM_HOOK_FAIL_LOUD_THRESHOLD;
...
export async function recordWorkerUnreachable(): Promise<number> {
  ...
  if (next.consecutiveFailures >= threshold) {
    ...
    emitBlockingError(
      `llm-mem worker unreachable for ${next.consecutiveFailures} consecutive hooks.`
    );
  }
...
const WORKER_FALLBACK_BRAND: unique symbol = Symbol.for('llm-mem/worker-fallback');
